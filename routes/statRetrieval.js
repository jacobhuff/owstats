...
module.exports = {
    ...
    insertUserData: async function(username, platform, con) {

        var allData = [];
        var currData = await getUserData(username, platform);
        ...
        con.query(sql, [allData], function (err, result, fields) {
            if (err) throw err;
            console.log(username + " USER inserted");
        });
    },

    insertHeroData: async function(username, platform, con) {
    
        // insert data into database
        var allData = [];
        var currData = await getHeroData(username, platform);
        allData.push(currData);
        ...
        for (var i = 0; i < allData.length; i++) {
            if (allData[i] !== null) {
                ...
                console.log(username + " HEROS inserted");
            }
        }
    }
}
...
async function getUserData(user, plat) {
    var currData = [];
    
    var currHTML = await ow.getRawHtmlFromBtag(user, plat);
    var compData = await ow.getModeStats(user, 'competitive', plat, currHTML);
    var generalData = await ow.getGeneralStats(user, plat, currHTML);
    var accountData = await ow.getAccountByName(user);
...
async function getHeroData(user, plat) {
    var data = [];
    var currData = [];
    heroestoInsert = [];
        ...
        if (compData.career_stats[allHeroes[i]] !== undefined
            ...
            switch (allHeroes[i]) {
                ...
                case "Brigitte":
                    ...
                    currData.push(converter.perTen(compData.career_stats[allHeroes[i]]["Hero Specific"].DamageBlocked, factor));
                    currData.push(converter.perTen(compData.career_stats[allHeroes[i]]["Hero Specific"].InspireUptimePercentage, factor));
                    currData.push(converter.perTen(compData.career_stats[allHeroes[i]]["Hero Specific"].SelfHealing, factor));
                    if (compData.career_stats[allHeroes[i]].Assists !== undefined) {
                        currData.push(converter.perTen(compData.career_stats[allHeroes[i]].Assists.DefensiveAssists, factor));
                        currData.push(converter.perTen(compData.career_stats[allHeroes[i]].Assists.OffensiveAssists, factor));
                ...
                case "Soldier: 76":
                    ...
                    currData.push(converter.perTen(compData.career_stats[allHeroes[i]]["Hero Specific"].HelixRocketKills, factor));
                    currData.push(converter.perTen(compData.career_stats[allHeroes[i]]["Hero Specific"].SelfHealing, factor));
